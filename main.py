...
class ScheduledOptim(object):
    ...
    def state_dict(self):
        self.optimizer.state_dict()  # 存放学习权重和偏执系数
    ...
    #
    # decay_steps：learning
    # rate更新的step周期，即每隔多少step更新一次learning rate的值
    #
...
def CoCPC_main(logger, args, train_loader, validation_loader, macro_loader):  # 这个函数没有用到
    print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
    global_timer = timer()  # global timer
    print('multivariable_num:', args.vr_num)

    if args.gpu:
        model = CoCPC(args.timestep, args.batch_size, args.seq_len, args.rep_dim, args.feat_dim, args.vr_num,
                      args.dataset).cuda()
    ...
    print('val loss mean: {:.4f}, val loss var:{:.4f}, val acc mean: {:.4f}, val acc var: {:.4f}'.format(
        np.mean(val_loss_list), np.var(val_loss_list), np.mean(val_acc_list), np.var(val_acc_list)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='ac118', help='choose dataset to run')
    ...
    parser.add_argument('--cpc_train', type=str, default='True',
                        help='whether to train our co-cpc model or prediction based on trained model')
    ...
    parser.add_argument('--n_warmup_steps', type=int, default=1000)#2022.3.25 Added by Zhao Lingzi
    ...
    if args.cpc_train == 'True':
        CoCPC_main(logger, args, train_loader[:30], validation_loader[:10],
                   macro_loader)  # just use small dataset for training Co-CPC

    ### load encoder model for prediction

    cpc_train = args.cpc_train

    cpc_model = CoCPC(args.timestep, args.batch_size, args.seq_len, args.rep_dim, args.feat_dim, args.vr_num,
                      args.dataset)
